...
from api.models import (
    User, FlashcardSet, Flashcard, Tag, FlashcardSetTag,
    FlashcardSetFavorite, FlashcardFavorite, FlashcardSetStats,
    FlashcardStatsSimple, FlashcardStatsStages, FlashcardStatsPercent
)
...
def test_login_user(api_client, test_user):
    test_user.set_password("password123")  # Ustaw hasło w odpowiednim formacie
    test_user.save()  # Zapisz zmiany w użytkowniku
...
def test_get_flashcard_sets(api_client, test_user):
    FlashcardSet.objects.all().delete()  # Wyczyszczenie bazy przed testem
    FlashcardSet.objects.create(user=test_user, name="Set 1", description="Description 1")
    FlashcardSet.objects.create(user=test_user, name="Set 2", description="Description 2")
...
def test_get_flashcard_set_details(api_client, test_user):
    flashcard_set = FlashcardSet.objects.create(user=test_user, name="Details Set", description="Detailed description")
...
def test_add_flashcard(api_client, test_user):
    flashcard_set = FlashcardSet.objects.create(user=test_user, name="Set for Flashcard")
...
def test_delete_flashcard(api_client, test_user):
    flashcard_set = FlashcardSet.objects.create(user=test_user, name="Set for Delete Test")
    flashcard = Flashcard.objects.create(set=flashcard_set, question="Q1", answer="A1")
...
def test_add_flashcard_set_to_favorites(api_client, test_user):
    flashcard_set = FlashcardSet.objects.create(user=test_user, name="Favorite Set")
    ...
    response = api_client.post(url, data, format='json')
    assert response.status_code == status.HTTP_201_CREATED
    assert FlashcardSetFavorite.objects.filter(user=test_user, set=flashcard_set).exists()

@pytest.mark.django_db
def test_add_flashcard_to_favorites(api_client, test_user):
    flashcard_set = FlashcardSet.objects.create(user=test_user, name="Set with Flashcard")
    flashcard = Flashcard.objects.create(set=flashcard_set, question="Q1", answer="A1")
    ...
    response = api_client.post(url, data, format='json')
    assert response.status_code == status.HTTP_201_CREATED
    assert FlashcardFavorite.objects.filter(user=test_user, flashcard=flashcard).exists()


@pytest.mark.django_db
def test_add_flashcard_set_stats(api_client, test_user):
    flashcard_set = FlashcardSet.objects.create(user=test_user, name="Stats Set")
    ...
    response = api_client.post(url, data, format='json')
    assert response.status_code == status.HTTP_201_CREATED
    assert FlashcardSetStats.objects.filter(set=flashcard_set, user=test_user).exists()
...
def test_get_flashcard_set_stats(api_client, test_user):
    ...
    stats = FlashcardSetStats.objects.create(
        ...
        total_study_time=timedelta(seconds=300)  # Użyj timedelta
    )
    url = reverse('get_flashcard_set_stats', args=[stats.id])
    response = api_client.get(url)
    assert response.status_code == status.HTTP_200_OK
    assert response.data["flashcards_viewed"] == 10
    assert response.data["total_study_time"] == 300  # Oczekuj wyniku w sekundach


@pytest.mark.django_db
def test_add_flashcard_stats_simple(api_client, test_user):
    flashcard_set = FlashcardSet.objects.create(user=test_user, name="Simple Stats Set")
    flashcard = Flashcard.objects.create(set=flashcard_set, question="Q1", answer="A1")
    ...
    response = api_client.post(url, data, format='json')
    assert response.status_code == status.HTTP_201_CREATED
    assert FlashcardStatsSimple.objects.filter(user=test_user, flashcard=flashcard).exists()

@pytest.mark.django_db
def test_update_flashcard_stats_simple(api_client, test_user):
    flashcard_set = FlashcardSet.objects.create(user=test_user, name="Simple Stats Set")
    flashcard = Flashcard.objects.create(set=flashcard_set, question="Q1", answer="A1")
    stats = FlashcardStatsSimple.objects.create(user=test_user, flashcard=flashcard, view_count=2)
...
def test_add_flashcard_stats_stages(api_client, test_user):
    flashcard_set = FlashcardSet.objects.create(user=test_user, name="Stages Stats Set")
    flashcard = Flashcard.objects.create(set=flashcard_set, question="Q1", answer="A1")
    ...
    response = api_client.post(url, data, format='json')
    assert response.status_code == status.HTTP_201_CREATED
    assert FlashcardStatsStages.objects.filter(user=test_user, flashcard=flashcard).exists()

@pytest.mark.django_db
def test_update_flashcard_stats_stages(api_client, test_user):
    flashcard_set = FlashcardSet.objects.create(user=test_user, name="Stages Stats Set")
    flashcard = Flashcard.objects.create(set=flashcard_set, question="Q1", answer="A1")
    stats = FlashcardStatsStages.objects.create(user=test_user, flashcard=flashcard, view_count=1, stage=1, learned=False)
...
def test_add_flashcard_stats_percent(api_client, test_user):
    flashcard_set = FlashcardSet.objects.create(user=test_user, name="Percent Stats Set")
    flashcard = Flashcard.objects.create(set=flashcard_set, question="Q1", answer="A1")
    ...
    response = api_client.post(url, data, format='json')
    assert response.status_code == status.HTTP_201_CREATED
    assert FlashcardStatsPercent.objects.filter(user=test_user, flashcard=flashcard).exists()

@pytest.mark.django_db
def test_update_flashcard_stats_percent(api_client, test_user):
    flashcard_set = FlashcardSet.objects.create(user=test_user, name="Percent Stats Set")
    flashcard = Flashcard.objects.create(set=flashcard_set, question="Q1", answer="A1")
    stats = FlashcardStatsPercent.objects.create(user=test_user, flashcard=flashcard, view_count=3, learning_stage="not_learned")
