...
from api.models import (
    FlashcardSet, Flashcard, Tag, FlashcardSetTag, FlashcardSetFavorite,
...
def test_create_flashcard_stats_simple():
    ...
    flashcard_set = FlashcardSet.objects.create(user=user, name="Fiszki z angielskiego")
    flashcard = Flashcard.objects.create(set=flashcard_set, question="What is an apple?", answer="A fruit.")
    stats = FlashcardStatsSimple.objects.create(user=user, flashcard=flashcard, view_count=10)
...
def test_flashcard_stats_simple_str():
    ...
    flashcard_set = FlashcardSet.objects.create(user=user, name="Fiszki z angielskiego")
    flashcard = Flashcard.objects.create(set=flashcard_set, question="What is an apple?", answer="A fruit.")
    stats = FlashcardStatsSimple.objects.create(user=user, flashcard=flashcard, view_count=5)

    expected_str = f"{user.username} - {flashcard.id} (Views: {stats.view_count})"
...
def test_flashcard_set_favorite():
    ...
    flashcard_set = FlashcardSet.objects.create(user=user, name="Fiszki z chemii")
    favorite = FlashcardSetFavorite.objects.create(user=user, set=flashcard_set)
...
def test_flashcard_favorite():
    ...
    flashcard_set = FlashcardSet.objects.create(user=user, name="Fiszki z biologii")
    flashcard = Flashcard.objects.create(set=flashcard_set, question="Co to jest DNA?", answer="Nośnik informacji genetycznej")
...
def test_flashcard_set_stats():
    ...
    flashcard_set = FlashcardSet.objects.create(user=user, name="Fiszki z fizyki")
    stats = FlashcardSetStats.objects.create(user=user, set=flashcard_set, flashcards_viewed=5)

    assert stats.user == user
    assert stats.set == flashcard_set
    assert stats.flashcards_viewed == 5
    assert str(stats) == f"{stats.id} - set: {flashcard_set.id} - user: {user.id}"


@pytest.mark.django_db(transaction=True)
def test_flashcard_stats_stages():
    user = User.objects.create(username="bartekJPG", password="password123")
    flashcard_set = FlashcardSet.objects.create(user=user, name="Fiszki z matematyki")
    flashcard = Flashcard.objects.create(set=flashcard_set, question="Ile to 2+2?", answer="4")
    stats = FlashcardStatsStages.objects.create(user=user, flashcard=flashcard, view_count=3, stage=2, learned=True)
    ...
    assert str(stats) == f"{user.username} - {flashcard.id} (Stage: {stats.stage}, Learned: {stats.learned})"


@pytest.mark.django_db(transaction=True)
def test_flashcard_stats_percent():
    user = User.objects.create(username="bartekPNG", password="password123")
    flashcard_set = FlashcardSet.objects.create(user=user, name="Fiszki z języka polskiego")
    flashcard = Flashcard.objects.create(set=flashcard_set, question="Co to jest metafora?", answer="Środek stylistyczny")
    stats = FlashcardStatsPercent.objects.create(user=user, flashcard=flashcard, view_count=2, learning_stage="still_learning")
